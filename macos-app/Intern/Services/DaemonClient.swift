...
/// Thread-safe client for communicating with the Intern daemon via Unix socket.
...
final class DaemonClient: @unchecked Sendable {
    private let queue = DispatchQueue(label: "com.intern.client", qos: .userInitiated)
    ...
    init() {
        ...
        socketPath = home.appendingPathComponent(".intern/daemon.sock").path
    }
    ...
    private func connectSync() throws {
        ...
        // Set socket read timeout to 5 minutes.
        // LLM-backed RPC calls (prompt.generate, prompt.optimize) can take
        // several minutes to complete. 30s was too aggressive and caused
        // spurious timeouts during generation.
    ...
    func listTasks() async throws -> [INTask] {
        return try await call("task.list")
    }

    func getTask(_ id: String) async throws -> INTask {
        return try await call("task.get", params: ["id": id])
    }

    func createTask(_ params: [String: Any]) async throws -> INTask {
        return try await call("task.create", params: params)
    }

    func updateTask(_ params: [String: Any]) async throws -> INTask {
    ...
    func pauseTask(_ id: String) async throws -> INTask {
        return try await call("task.pause", params: ["id": id])
    }

    func resumeTask(_ id: String) async throws -> INTask {
    ...
    func importTask(_ export: TaskExport) async throws -> INTask {
