    ...
    var body: some View {
        LazyVGrid(columns: columns, spacing: 12) {
            ...
            MetricCard(
                ...
                accent: metrics.totalRuns == 0 ? .inTextPrimary : (metrics.overallSuccessRate >= 95 ? .inGreen : (metrics.overallSuccessRate >= 80 ? .inAmber : .inRed))
            )
            MetricCard(
                label: "Total Spend",
            ...
            MetricCard(
                ...
                sub: isConnected
                    ...
                    : "not running",
                accent: isConnected ? .inGreen : .inRed
            )
        }
        .padding(.vertical, 16)
