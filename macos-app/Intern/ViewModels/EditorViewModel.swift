...
/// ViewModel that owns the persistent editor tab state for Intern.
///
...
class EditorViewModel: ObservableObject {
    ...
    @Published var draft: INTaskDraft = INTaskDraft()
    ...
    private var originalSnapshot: INTaskDraft?
    ...
    func loadTask(_ task: INTask) {
        editorState = .editing(taskId: task.id)
        let d = INTaskDraft(from: task)
        draft = d
        originalSnapshot = d
        taskId = task.id
        error = nil
    ...
    func loadFromImportedCommand(_ command: ClaudeCommand) {
        editorState = .creating
        let d = INTaskDraft(from: command)
        draft = d
        originalSnapshot = nil
        taskId = nil
        error = nil
