...
class TaskEditorViewModel: ObservableObject {
    @Published var draft: INTaskDraft
    ...
    init(isNew: Bool, task: INTask? = nil) {
        self.isNew = isNew
        self.taskId = task?.id
        if let task = task {
            self.draft = INTaskDraft(from: task)
        } else {
            self.draft = INTaskDraft()
        }
    }
    ...
    init(isNew: Bool, importedCommand: ClaudeCommand) {
        self.isNew = true
        self.taskId = nil
        self.draft = INTaskDraft(from: importedCommand)
    }
    ...
    func applyTemplate(_ template: TaskTemplate) {
        draft = INTaskDraft(from: template)
