...
final class NotificationManager: NSObject, ObservableObject {
    ...
    private func registerCategories() {
        let viewLogs = UNNotificationAction(identifier: "LC_VIEW_LOGS", title: "View Logs", options: [.foreground])
        let runAgain = UNNotificationAction(identifier: "LC_RUN_AGAIN", title: "Run Again", options: [.foreground])
    ...
    func sendTaskSuccess(taskId: String, taskName: String, durationSecs: Int, costUsd: Double?) {
        ...
        if let cost = costUsd, cost > 0 {
            body = "Completed \(duration). Cost: \(formatCost(cost))"
        } else {
            body = "Completed \(duration)."
        }
