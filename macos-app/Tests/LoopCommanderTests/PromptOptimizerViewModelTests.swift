...
// executableTarget. Until the LoopCommander executable is split into a
// LoopCommanderLib library + thin LoopCommander executable, these tests
// cannot use `@testable import LoopCommander`.
//
// To make the split:
//   1. Move all source files except LoopCommanderApp.swift (the @main entry
//      point) into a new `.target(name: "LoopCommanderLib", path: "LoopCommander")`
//      library target, excluding the entry point file.
//   2. Create a `.executableTarget(name: "LoopCommander", ...)` whose path
//      contains only LoopCommanderApp.swift, with a dependency on
//      LoopCommanderLib.  Any types used from the library must be `public`.
//   3. Change the `.testTarget(name: "LoopCommanderTests", ...)` dependency
//      from [] to ["LoopCommanderLib"].
//   4. Replace the stub declarations below with:
//        @testable import LoopCommanderLib
//      and delete the local stub types.
//
// Until then, run `swift test` from macos-app/ to see the following diagnostic:
//   error: testTarget 'LoopCommanderTests' cannot depend on
//   executableTarget 'LoopCommander'
//
...
// MARK: - Local Stubs (replace with @testable import LoopCommanderLib)
//
...
/// Minimal LCTaskDraft stub for applyOptimization tests.
struct LCTaskDraft {
    var command: String = ""
}
...
// LoopCommanderLib is available, delete this stub and use the real class.
...
class PromptOptimizerViewModel: ObservableObject {
    ...
    func applyOptimization(to draft: inout LCTaskDraft) {
