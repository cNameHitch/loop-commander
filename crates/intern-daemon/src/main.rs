//! Intern daemon entry point.
//!
//! The daemon is the sole API server for Intern. Both the Swift macOS
...
use intern_core::{DaemonEvent, InternPaths, TaskStatus};
...
async fn main() -> Result<()> {
    ...
    info!(version = VERSION, "Starting Intern daemon");
    ...
    //    InternPaths does not implement Clone, so we construct separate instances
    //    for each component that needs one.
    let config_paths = InternPaths::new();
    let scheduler_paths = InternPaths::new();
    ...
    // 7. Sync tasks to launchd: for each Active task, ensure plist is registered + loaded.
    {
        ...
        // Generate missing context files for all existing tasks (lazy migration).
        for task in &tasks {
            let sanitized = intern_config::sanitize_task_name(&task.name);
            let commands_dir = task.working_dir.join(".claude").join("commands");
            ...
            if !primary.exists() && !suffixed.exists() {
                if let Err(e) = cfg.write_command_file(task) {
                    warn!(
                        task_id = %task.id,
                        error = %e,
                        "Failed to generate context file during startup migration"
                    );
                }
            }
        }
        info!("Context file migration pass complete");
    }
...
fn check_single_instance(paths: &InternPaths) -> Result<()> {
    // Check PID file.
